...
class WallTraceTest(unittest.TestCase):
    ...
    def test_io(self):
        ...
        left, right = self.set_and_get(0,5,1000,0) #side is not a trigger of stop
        self.assertTrue(left == right !=0,"stop wrongly by side sensors")

        left, right = self.set_and_get(0,10,0,0) #left turn
        self.assertTrue(left < right,"don't curve to left")

        left, right = self.set_and_get(0,200,0,0) #right turn
        self.assertTrue(left > right == 0,"don't curve to right")

        left, right = self.set_and_get(0,5,0,0) #out of control at far from wall
